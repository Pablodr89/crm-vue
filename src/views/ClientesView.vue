<script setup>
    ...
    let existenClientes = computed(() => {
        return clientes.value.length > 0
    })

    let cambiarEstado = ({id, estado}) => {
        ClienteService.changeEstado(id, {estado: !estado})
            .then(() => {
                let i = clientes.value.findIndex(cliente => cliente.id === id)
