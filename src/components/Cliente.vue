<script setup>
    ...
    let nombreCliente = computed(() => {
        return props.cliente.nombre + ' ' + props.cliente.apellido
    })

    let estadoCliente = computed(() => {
        return props.cliente.estado
    })

    let emit = defineEmits(['cambiar-estado', 'eliminar-cliente'])
</script>
...
<template>
    <tr>
        ...
        <td class="whitespace-nowrap px-3 py-4 text-sm">
           <button class="inline-flex rounded-full px-2 text-xs font-semibold leading-5" :class="[estadoCliente ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800']" @click="$emit('cambiar-estado', {id: cliente.id, estado: cliente.estado})">{{ estadoCliente ? 'Activo' : 'Inactivo' }}</button>
        </td>
        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 ">
